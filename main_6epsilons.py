...
def train(args, model, device, train_loader, optimizer, epoch, criterion, oracle, test_loader_constr, test_loader_adv, print_iter, train_writer=None, pruning_engine=None):
...
##########################################################################################################################
        ...
        if print_iter % args.log_interval == 0:
            ...
            print('{0}, '
                  ...
                  '{adv_acc6:.4f}, '
                  .format(
                      epoch, neuron_units , loss=losses, dl2_loss=test_loss_dl2,
            ...
            #*k
            if epoch > args.print_conf_after_epoch and args.confusion:
                precision_dl2, constr_acc, test_loss_dl2 = test_constraints(args, oracle, test_loader_constr, model, device, epoch, pruning_engine)
                # confusion matrix                               
                y_true = []
                y_pred = []
                for i, (x_batch, y_batch) in enumerate(train_loader):
                    x = model(x_batch).detach().numpy()
                    y = y_batch.detach().numpy()
                    
#                    p_np = []
#                    t_np = []
                    for i in range(len(x)):
#                        p_np.append(np.argmax(x[i]))
#                        t_np.append(y[i])
                        y_true.append(np.argmax(x[i]))
                        y_pred.append(y[i])
...
#                print("return from training step, unsafe and was not tested correctly")
    ...
    if neurons_left == 0:
        exit()
    return neurons_left, print_iter
...
#*k segment constraint done with pca
...
#            #get output                                                   #                                            
#            output = model(x_batches[i])
#            pred = output.max(1, keepdim=True)[1]
#            true_class = pred.eq(y_batches[i].view_as(pred))
#        
#            for j in range(k):
#                if true_class[k-j-1] == False:
#                    x_batches[i] = torch.cat([x_batches[i][:k-j-1], x_batches[i][k-j:]])
#                    y_batches[i] = torch.cat([y_batches[i][:k-j-1], y_batches[i][k-j:]])
#        
#        #if sizes of 2 vectors differ...
#        size0 = y_batches[0].size()[0]    
#        size1 = y_batches[1].size()[0]
#        if size0 > size1:
#            size_diff = size0 - size1
#            x_batches[0] = torch.cat([x_batches[0][:size0-size_diff], x_batches[0][size0:]])
#            y_batches[0] = torch.cat([y_batches[0][:size0-size_diff], y_batches[0][size0:]])
#        if size1 > size0:
#            size_diff = size1 - size0
#            x_batches[1] = torch.cat([x_batches[1][:size1-size_diff], x_batches[1][size1:]])
#            y_batches[1] = torch.cat([y_batches[1][:size1-size_diff], y_batches[1][size1:]])
#
#        size0 = y_batches[0].size()[0]
#        k = size0
#        
...
#    print(" ".join(sys.argv))
    ...
    if args.pruning:
        ...
        run_params = "%s_%s_%d_%d_%s_%s_%s" % (args.dataset, args.model, pruning_settings['prune_per_iteration'], pruning_settings['frequency'], args.dl2_weight, args.constraint.replace(" ", "_").replace("(","_").replace(")","_"), args.attack_method)
        print(run_params)
...
#    print(" ".join(sys.argv))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if not args.run_test and not args.get_inference_time:
            num_left, new_iter = train(args, model, device, train_loader, optimizer, epoch, criterion, oracle, test_loader_constr, test_loader_adv, print_iter, train_writer=train_writer, pruning_engine=pruning_engine)
