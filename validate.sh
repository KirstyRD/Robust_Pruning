...
python3 validate.py --dataset=$dataset --model=$network --tensorboard=True  --pruning_config=$config --constraint="${dl2_constr}" --dl2-weight=$dl2_weight  --batch-size=$batch_size --name="CIFAR100_res_valid" --checkpoint_folder="CIFAR100_res" --log_file=$log_file --starting_line=$starting_line
