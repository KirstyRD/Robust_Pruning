...
def validate(args, test_loader, model, device, criterion):
    ...
    with torch.no_grad():
        ...
        #print("time taken for %d iterations, per-iteration is: "%(int_i), (end_get_inference_time -start_get_inference_time)*1000.0/float(int_i), "ms")
        time_all = (end_get_inference_time - start_get_inference_time)*1000.0
        ...
        #    int_i += 1
        #end_get_inference_time = time.time()
        #time_no_input = (end_get_inference_time - start_get_inference_time)*1000.0

        #time_1_input = (time_all - time_no_input)/float(int_i)
...
#    print(' * Prec@1 {top1.avg:.3f}, Prec@5 {top5.avg:.3f}, Time {batch_time.sum:.5f}, Loss: {losses.avg:.3f}'.format(top1=top1, top5=top5,batch_time=batch_time, losses = losses) )
    ...
    return top1.avg, losses.avg, #time_1_input
...
#    print("Total number of parameters, w/o usage of bn consts: ", total_size_params)
    ...
    #i, y_iter[i], train_loss=y_loss[i], train_dl2_loss=y_dl2_loss[i],train_top1=y_top1[i], train_top5=y_top5[i], test_loss=loss , test_acc =top1, inf_time=inf_time
    print("print_iter, train_iter, train_loss, train_dl2_loss, train_top1, train_top5,  test_loss,  test_acc, inf_time")
    for j in range(len(y_params)-args.starting_line):
        ...
        checkpoint_file3 = "%s/models/checkpoint_%s_%s_%s_%s_%s_%s_%s_%s_%.3f500.weights" % (args.checkpoint_folder, args.dataset, args.model, pruning_settings['prune_per_iteration'], pruning_settings['frequency'], args.dl2_weight, args.constraint.replace(" ", "_").replace("(","_").replace(")","_"), y_params[i], y_iter[i], lower_top )
#        print("Finding ", checkpoint_file1)
        ...
        top1, loss = validate(args, test_loader_constr, model, device, criterion)    
...
#              '{inf_time} '
              .format(
                  i, y_iter[i], train_loss=y_loss[i], train_dl2_loss=y_dl2_loss[i],
                  train_top1=y_top1[i], train_top5=y_top5[i], 
                  test_loss=loss , test_acc =top1#, inf_time=inf_time
              ))
